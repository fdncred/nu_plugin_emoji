...
impl Emoji {
    ...
    #[inline]
    pub const fn codepoints(&self) -> &str {
        self.codepoints
    }
    ...
    pub fn skin_tones(&self) -> Option<impl Iterator<Item = &'static Self>> {
        ...
        Some(gen::EMOJIS[i as usize..].iter().take(n as usize))
    }
...
pub fn get(s: &str) -> Option<&'static Emoji> {
    gen::unicode::MAP.get(s).map(|&i| &gen::EMOJIS[i])
}
...
pub fn get_by_shortcode(s: &str) -> Option<&'static Emoji> {
    gen::shortcode::MAP.get(s).map(|&i| &gen::EMOJIS[i])
}
